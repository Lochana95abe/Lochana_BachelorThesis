...
class SimulatorEngine:
    def __init__(self, app, config, scenario, num_pictures=10):
        ...
        self.normal_deadline = norm * 1.2
        if "Failover" in scenario: self.active_deadline = (norm + config.fail_detect_ms + config.fail_replan_ms) * 1.2
    ...
    def ev_send_final_done(self, pic):
        ...
        delay = calc_data_ms(self.config.final_tensor_mb, self.config.link_rate_mbps)
        self.log_step(pic, f"Send Final ({p1.name}->Edge)", p1.name, "Edge", "Data", f"{self.config.final_tensor_mb} MB", delay, "To coordinator")
        self.set_idle(p1)
