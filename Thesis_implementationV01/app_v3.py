...
class Config:
    def __init__(self):
        ...
        self.edge_only_full_compute_ms = 15.3
...
class SimulatorEngine:
        ...
        for pic in self.pictures:
            ...
            if pic.state == "INIT":
                active_requiring = sum(1 for p in self.pictures if p.state not in [
                    "INIT", "DONE", 
                    "SEND_T2", "PEER2_PROC", "WAIT_EDGE", "SEND_FINAL", "EDGE_FINISH", "ACK",
                ...
                if peer:
                    p1 = pic.roles["Peer1"]
                    self.set_busy(p1, "TRANSMITTING", pic.id, "Send T2")
                    self.set_busy(peer, "RECEIVING", pic.id, "Recv T2")
                    pic.roles["Peer2"] = peer
                    pic.state = "SEND_T2"
                    self.app.log_msg(f"{pic.id} selected {peer.name} as Peer 2.")
                ...
                if self.edge.state == "IDLE":
                    p2 = pic.roles["Peer2"]
                    self.set_busy(p2, "TRANSMITTING", pic.id, "Send Fin")
                    self.set_busy(self.edge, "RECEIVING", pic.id, "Send Fin")
    ...
    # ------------------ EVENT CALLBACKS ------------------
    def log_step(self, pic, step_name, sender, receiver, stype, payload, delay, note=""):
        pic.cum_time += delay
        pic.step_counter += 1
    ...
    def ev_transfer_failed(self, pic):
        ...
        self.log_step(pic, f"Tensor 1 ({cap.name}->{failed_peer.name})", cap.name, failed_peer.name, "Data", f"{self.config.tensor1_mb} MB", delay, f"{failed_peer.name} unresponsive")
    ...
    def ev_fail_detect_done(self, pic):
        cap = pic.roles["Capture"]
        self.log_step(pic, "Failure Detect", "System", "System", "Control", "-", self.config.fail_detect_ms, "Detected peer failure")
    ...
    def ev_replan_done(self, pic):
        self.log_step(pic, "Reroute Plan", "Edge", "Edge", "Compute", "-", self.config.fail_replan_ms, "Reroute planned")
    ...
    def ev_req_done(self, pic):
        ...
        delay = calc_control_ms(self.config.control_msg_kb, self.config.link_rate_mbps)
        self.log_step(pic, "Task Request", cap.name, "Edge", "Control", f"{self.config.control_msg_kb} KB", delay, "Requesting plan")
    ...
    def ev_plan_gen_done(self, pic):
        self.log_step(pic, "Global Plan Gen", "Edge", "Edge", "Compute", "-", self.config.edge_plan_ms, "Plan generated")
        cap = pic.roles["Capture"]
    ...
    def ev_plan_dist_done(self, pic):
        delay = calc_control_ms(self.config.control_msg_kb, self.config.link_rate_mbps)
        self.log_step(pic, "Plan Dist", "Edge", "Nodes", "Control", f"{self.config.control_msg_kb} KB", delay, "Plan received")
        self.set_idle(self.edge)
        cap = pic.roles["Capture"]
    ...
    def ev_cap_proc_done(self, pic):
        ...
        self.log_step(pic, f"Seg1 Proc ({cap.name})", cap.name, cap.name, "Compute", "-", self.config.capture_processing_ms, "Captured & processed")
    ...
    def ev_send_t1_done(self, pic):
        ...
        delay = calc_data_ms(self.config.tensor1_mb, self.config.link_rate_mbps)
        self.log_step(pic, f"Tensor 1 ({cap.name}->{peer.name})", cap.name, peer.name, "Data", f"{self.config.tensor1_mb} MB", delay, "Sent onward")
        self.set_idle(cap)
    ...
    def ev_peer1_proc_done(self, pic):
        ...
        self.log_step(pic, f"Seg2 Proc ({peer.name})", peer.name, peer.name, "Compute", "-", self.config.peer1_processing_ms, "Segment 2 processed")
        self.set_busy(peer, "WAITING", pic.id, "Wait P2")
    ...
    def ev_send_t2_done(self, pic):
        ...
        delay = calc_data_ms(self.config.tensor2_mb, self.config.link_rate_mbps)
        self.log_step(pic, f"Tensor 2 ({p1.name}->{p2.name})", p1.name, p2.name, "Data", f"{self.config.tensor2_mb} MB", delay, "Sent onward")
        self.set_idle(p1)
        self.set_busy(p2, "PROCESSING", pic.id, "Seg3")
        self.app.log_msg(f"{pic.id} Tensor2 sent. {p1.name} is now available. {pic.id} Seg3 running on {p2.name}.")
    ...
    def ev_peer2_proc_done(self, pic):
        ...
        self.log_step(pic, f"Seg3 Proc ({p2.name})", p2.name, p2.name, "Compute", "-", self.config.peer2_processing_ms, "Segment 3 processed")
        self.set_busy(p2, "WAITING", pic.id, "Wait Edge")
        pic.state = "WAIT_EDGE"
    ...
    def ev_send_final_done(self, pic):
        ...
        delay = calc_data_ms(self.config.final_tensor_mb, self.config.link_rate_mbps)
        self.log_step(pic, f"Send Final ({p2.name}->Edge)", p2.name, "Edge", "Data", f"{self.config.final_tensor_mb} MB", delay, "To coordinator")
        self.set_idle(p2)
        self.set_busy(self.edge, "PROCESSING", pic.id, "Edge Fin")
        self.app.log_msg(f"{pic.id} Final Tensor sent. {p2.name} is now available.")
        self.add_event(self.config.edge_finish_ms, lambda p=pic: self.ev_edge_finish_done(p))

    def ev_edge_finish_done(self, pic):
        self.log_step(pic, "Edge Finish", "Edge", "Edge", "Compute", "-", self.config.edge_finish_ms, "Model finished")
    ...
    def ev_ack_done(self, pic):
        ...
        self.log_step(pic, "Completion Ack", "Edge", cap_name, "Control", f"{self.config.control_msg_kb} KB", delay, "Done")
        self.set_idle(self.edge)
        pic.state = "DONE"
    ...
    def ev_edge_only_plan_done(self, pic):
        self.log_step(pic, "Edge Plan (Fallback)", "Edge", "Edge", "Compute", "-", self.config.edge_plan_ms, "Plan generated")
        cap = pic.roles["Capture"]
    ...
    def ev_send_raw_done(self, pic):
        ...
        cap.drain(self.config.tensor1_send_drain)
        delay = calc_data_ms(self.config.raw_image_mb, self.config.link_rate_mbps)
        self.log_step(pic, f"Send Raw Image ({cap.name})", cap.name, "Edge", "Data", f"{self.config.raw_image_mb} MB", delay, "Offloaded")
        self.set_idle(cap)
    ...
    def ev_edge_full_done(self, pic):
        self.log_step(pic, "Edge Full Process", "Edge", "Edge", "Compute", "-", self.config.edge_only_full_compute_ms, "Finished locally at Edge")
        self.set_busy(self.edge, "TRANSMITTING", pic.id, "Ack")
        delay = calc_control_ms(self.config.control_msg_kb, self.config.link_rate_mbps)
        self.add_event(delay, lambda p=pic: self.ev_ack_done(p))
    ...
    def ev_local_detect_done(self, pic):
        cap = pic.roles["Capture"]
        self.log_step(pic, "Detect Disconnect", cap.name, cap.name, "Control", "-", self.config.fail_detect_ms, "No Edge/Peers")
        self.set_busy(cap, "PROCESSING", pic.id, "Local Proc")
        
        local_time = self.config.capture_processing_ms + self.config.peer1_processing_ms + self.config.peer2_processing_ms
        self.add_event(local_time, lambda p=pic: self.ev_local_proc_done(p, local_time))

    def ev_local_proc_done(self, pic, local_time):
        cap = pic.roles["Capture"]
        cap.drain(self.config.capture_segment_processing_drain + self.config.peer1_segment_processing_drain + self.config.peer2_segment_processing_drain)
        self.log_step(pic, f"Local Full Process ({cap.name})", cap.name, cap.name, "Compute", "-", local_time, "Fully local execution")
        self.set_idle(cap)
        pic.state = "DONE"
        self.log_step(pic, "Local Complete", cap.name, cap.name, "Compute", "-", 0.0, "Done")
        self.print_pic_summary(pic)
...
# ==========================================
class UAVDemoAppV3(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Pipelined UAV Swarm Inference Demo V3")
